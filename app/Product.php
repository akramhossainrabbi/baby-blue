...
class Product extends Model
{
    ...
    public function getImageUrlAttribute()
    {
        if (!empty($this->image)) {
            ...
            $image_url = SM::sm_get_the_src($this->image);
        } else {
    ...
    public static function productsDropdown($business_id, $prepend_none = true, $append_id = true)
    {
        $all_contacts = Product::where('business_id', $business_id)
                        ->active();
        ...
        $products = $all_contacts->pluck('name', 'id');

        //Prepend none
        if ($prepend_none) {
            $products = $products->prepend(__('lang_v1.none'), '');
        }
    ...
    {
        return $this->hasOne(\App\Variation::class);
    }
    /**
