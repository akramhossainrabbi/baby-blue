...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'card_type' => !empty($input['card_type']) ? $input['card_type'] : null,
            'transaction_id' => !empty($input['transaction_id']) ? $input['transaction_id'] : null,
            'ecom_order_id' => !empty($input['ecom_order_id']) ? $input['ecom_order_id'] : null,
        ]);
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        // Ecommerce Info
        $output['card_type'] = $transaction->card_type;
        $output['transaction_id'] = $transaction->transaction_id;
        $output['ecom_order_id'] = $transaction->ecom_order_id;
        ...
        if ($transaction_type == 'sell') {
            ...
            $return_lines = $transaction->sell_lines;
            ...
            $details_return = $this->_receiptDetailsSellReturnLines($return_lines, $il, $business_details);
            $output['return_lines'] = $details_return['lines'];
     ...
     * @return array
     */
    public function _receiptDetailsSellReturnLines($lines, $il, $business_details)
    {
        $is_lot_number_enabled = $business_details->enable_lot_number;
        $is_product_expiry_enabled = $business_details->enable_product_expiry;
    ...
    */
     ...
     * @return Obj
     */
    public function getExpenseReport($business_id,$filters = [],$type = 'by_category') 
    {
    ...
    public function calculatePaymentStatus($transaction_id, $final_amount = null)
    {
        ...
        $business_details = $this->getDetails($business_id);
        ...
        $invoice_layout = $this->invoiceLayout($business_id, $location_id, $invoice_layout_id);
...
// 		dd($return_amount);	   
        ...
        if($return_amount>0)
        {
            $final_amount = $final_amount - $return_amount;
        }
    ...
    */
    public function getListSells($business_id)
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->leftJoin('transaction_sell_lines as tsl', 'transactions.id', '=', 'tsl.transaction_id')
                ->leftJoin('transaction_sell_lines_purchase_lines as tslp', 'tsl.id', '=', 'tslp.sell_line_id')
                ->leftJoin('purchase_lines as pl', 'tslp.purchase_line_id', '=', 'pl.id')
                ->leftJoin('users as u', 'transactions.created_by', '=', 'u.id')
                ...
                ->select(
                    ...
                    DB::raw('(SELECT SUM(pl.purchase_price_inc_tax*tslp.quantity)  ) as final_purchase'), 
