...
class ReportController extends Controller
{
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (!empty($report_type)) {
                ...
                {
                    $query->where('t.is_direct_sale', 0);
                }

                if($report_type == 'online')
                {
                    $query->where('t.is_direct_sale', 1);
                }
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('final_purchase', function ($row) {
                    return '<span class="display_currency final-purchase" data-currency_symbol="true" data-orig-value=" ' .$row->final_purchase . ' ">' .$row->final_purchase. ' </span>';
                })
                
                ->addColumn('action', '<button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action("TransactionPaymentController@viewPayment", [$DT_RowId]) }}">@lang("messages.view")
                    </button> @if(!empty($document))<a href="{{asset("/uploads/documents/" . $document)}}" class="btn btn-success btn-xs" download=""><i class="fa fa-download"></i> @lang("purchase.download_document")</a>@endif')
                ->rawColumns(['invoice_no', 'amount',  'final_purchase', 'method', 'action'])
                ->make(true);
        }
        $business_locations = BusinessLocation::forDropdown($business_id);
    ...
    public function getProductLedger()
    {
        ...
        if (request()->ajax()) {
            ...
            // $query = Product::leftJoin('purchase_lines as PL', 'PL.product_id', '=', 'products.id')
            ...
            return Datatables::of($query)
                ...
                ->editColumn('report_type', function ($row) {
                ...
                ->rawColumns(['subtotal', 'selling_price', 'quantity', 'purchase_price', 'sale_invoice_no', 'purchase_ref_no','report_type'])
                ->make(true);
        }

        $suppliers = Contact::suppliersDropdown($business_id, false);
        $customers = Contact::customersDropdown($business_id, false);
        $products = Product::productsDropdown($business_id, false);

        $business_locations = BusinessLocation::forDropdown($business_id);
        return view('report.product_ledger')->with(compact('suppliers', 'customers', 'business_locations','products'));
    }


    public function getEcommerceProductWiseSellReport()
    {
        dd("HI");
    }
