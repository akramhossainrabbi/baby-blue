...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        //User management dropdown
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('user.view')) {
                            $sub->url(
                                action('AppearanceController@smthemeoptions'),
                                "Nptl Theme Options",
                        ...
                        if (auth()->user()->can('roles.view')) {
                            $sub->url(
                                action('AppearanceController@menus'),
                                "Menus",
                        ...
                        //     $sub->url(
                        //         action('AppearanceController@editor'),
                        //         "Editor",
            ...
            $menu->addHeader('Ecommerce Manage',10);
            //Contacts dropdown
            ...
            //Common
            $menu->addHeader('Common',15);

            //Media 
            $menu->url(action('MediaController@index'), "Media", ['icon' => 'fa fas fa-tachometer-alt', 'active' => request()->segment(1) == 'home'])->order(15);
            //Manage Order

            $menu->url(action('OrdersController@index'), "Ecommerce Order", ['icon' => 'fa fas fa-tachometer-alt', 'active' => request()->segment(1) == 'home'])->order(15);
...
//                        if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create')) {
                    $sub->url(
                        ...
                        'Tag',
                        ['icon' => 'fa fas fa-gem', 'active' => request()->segment(1) == 'tags']
                    );
//                        }
...
//                        if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create')) {
                    $sub->url(
                        ...
                        'Blog',
                        ['icon' => 'fa fas fa-gem', 'active' => request()->segment(1) == 'bloglist']
                    );
//                        }
//                    if (auth()->user()->can('purchase.update')) {
                    $sub->url(
                        action('SlidersController@index'),
                        'Slider',
                        ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'sliderslist']
                    );
//                    }
//                    if (auth()->user()->can('purchase.update')) {
                    $sub->url(
...
//                    if (auth()->user()->can('purchase.update')) {
                    $sub->url(
                        ...
                        'Page',
                        ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'pageslist']
                    );
//                    }
//                    if (auth()->user()->can('purchase.update')) {
                    $sub->url(
                        action('AppearanceController@smthemeoptions'),
                        'Apperance',
                        ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'smthemeoptions']
                    );
//                    }
//                    if (auth()->user()->can('purchase.update')) {
                    $sub->url(
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('sell.purchase_view')) {
                            $sub->url(
                                action('SellController@getSellWithPurchase'),
                                __('Sales With Purchase'),
                                ['icon' => 'fa fas fa-arrow-circle-up', 'active' => request()->segment(2) == 'report-product-ledger']
                            );
                        }

                        if (auth()->user()->can('sell.purchase_view')) {
                            $sub->url(
                                action('ReportController@getProductLedger'),
                                __('Product Leadger'),
                                ['icon' => 'fa fas fa-arrow-circle-up', 'active' => request()->segment(2) == 'report-purchase-sells']
                            );
                        }
