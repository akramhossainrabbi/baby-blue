  ...
  <div class="row">
    <div class="col-md-6">
      <div class="m-6 mt-5 hidden-xs">
        <p><strong>@lang('sale.location'):</strong> @if(!empty($default_location->name)) {{$default_location->name}}
          @elseif(!empty($transaction->location_id)) {{$transaction->location->name}} @endif
          {{ @format_datetime('now') }} <i class="fa fa-keyboard hover-q text-muted" aria-hidden="true"
            data-container="body" data-toggle="popover" data-placement="bottom"
            data-content="@include('sale_pos.partials.keyboard_shortcuts_details')" data-html="true"
            data-trigger="hover" data-original-title="" title=""></i></p>
      </div>
    </div>
    <div class="col-md-6">
