...
<div class="modal-body">
    ...
    <div class="row">
      <div class="col-sm-4">
         @if($sell->ecom_order_id)
        <b>Ecom. Order ID:</b> #{{ $sell->ecom_order_id }}<br>  
        @endif
        ...
        @endif
        @if($sell->card_type )
        <b>Card Type:</b> {{ $sell->card_type }}<br>
        @endif
        @if($sell->transaction_id )
        <b>Transaction ID:</b> {{ $sell->transaction_id }}<br>
        @endif
      </div>
      <div class="col-sm-4">
    ...
    <div class="row">
      ...
      <div class="col-sm-12 col-xs-12">
        <div class="table-responsive">
          @include('sale_pos.partials.sale_line_return_details')
        </div>
      </div>
    </div>
          ...
          <table class="table bg-gray">
            <tr>
			  ...
			  @php
                 ...
                 $total_before_tax =  explode(" ",$sell->total_before_tax);
                 $total_before_tax = $total_before_tax[0];
                  if (str_contains($total_before_tax, ',')) 
                  {
                      $total_before_tax = str_replace(",","",$total_before_tax);
                  }
    			  $total_before_tax = (float)$total_before_tax - (float)$return_amount;
            ...
            <tr>
              ...
              @php
                 ...
                 $final_total =  explode(" ",$sell->final_total);
                 $final_total = $final_total[0];
                  if (str_contains($final_total, ',')) 
                  {
                      $final_total = str_replace(",","",$final_total);
                  }
    			  $final_total = (float)$final_total - (float)$return_amount;
    			  $final_total = (float)$final_total ;
              @endphp
              <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $final_total }}</span></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td>
                ...
                @endphp
                <span class="display_currency pull-right" data-currency_symbol="true" >{{ $final_total - $total_paid }}</span></td>
            </tr>
          </table>
        </div>
      </div>
