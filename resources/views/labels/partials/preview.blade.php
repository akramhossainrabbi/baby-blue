...
@foreach($product_details as $details)
	@while($details['qty'] > 0)
		...
		<div style="display:inline-block;vertical-align:middle;line-height:16px !important;font-family: Arial, Helvetica, sans-serif;">
			{{-- Business Name --}}
			@if(!empty($print['business_name']))
				<b style="display: block !important;font-size:14px;font-weight:bold;" class="text-uppercase">{{$business_name}}</b>
			@endif

			{{-- Product Name --}}
			@if(!empty($print['name']))
				<span style="display: block !important;font-size:12px;font-weight:bold;">
			...
			@if(!empty($print['price']))
				<b style="font-size:12px">@lang('lang_v1.price'):
				<!--{{session('currency')['symbol'] ?? ''}}-->
				TK. 

				@if($print['price_type'] == 'inclusive')
					{{@num_format($details['details']->sell_price_inc_tax)}} (+VAT)
				@else
					{{@num_format($details['details']->default_sell_price)}} (+VAT)
				@endif
				</b>
			@endif
